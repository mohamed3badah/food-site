...
function Dish(props) {
    ...
    if (loveData.includes(parseInt(dish["idour_dishes"]))) {
      loveDom.push(
        <i
          ...
          key={`dish-love-${dish["idour_dishes"]}`}
        ></i>
      );
    } else {
      loveDom.push(
  ...
  return (
    <div className="box" key={`dish-${dish["idour_dishes"]}`}>
      {ifLove(props.loveData)}
      <img
        src={`/images/${dish["img"]}`}
        alt=""
        key={`img-${dish["idour_dishes"]}`}
      />
      <h3>{dish["name"]}</h3>
      <div className="stars">{f(dish["stars"])}</div>
      <span>{`$${dish["price"]}`}</span>
      <button
        onClick={() => props.cart(dish["idour_dishes"], "dishes")}
        className="btn"
      >
        add to cart
      </button>
